...
void printContainer(const Container& cont){
    std::for_each(cont.begin(),cont.end(),[](const auto& elem){
        std::cout<<elem<<' ';
    });
    std::cout<<std::endl;
}
...
void printContainer(const std::map<K,V>& cont){
    std::for_each(cont.begin(),cont.end(),[](const auto& pair){
...
int main(){
    ...
    for(int i = (int)factors.size() - 1; i >= 0; i--){
        if(i < (int)factors.size() - 1) { std::cout << " * "; }
        std::cout << factors[i];
    }
    ...
    MyStack<int> tmp;
    int v = 0;
    while(stack.pop(v)) { tmp.push(v); }
    std::vector<int> demo_vec;
    while(tmp.pop(v)) { demo_vec.push_back(v); }
    ...
    std::cout << "vector: "; printContainer(demo_vec);
    std::cout << "list:   "; printContainer(demo_list);
    std::cout << "map:    "; printContainer(demo_map);
